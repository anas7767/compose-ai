...
from compose_ai_api.domains.ai_architect.execution import process_brief_job
from compose_ai_api.domains.ai_architect.models import AIJob, AIJobStatus
from compose_ai_api.domains.infrastructure.models import WorkerKind


async def run_worker() -> None:
    settings = get_settings()
    worker_id = default_worker_id(WorkerKind.AI_ARCHITECT)
    heartbeat_task = asyncio.create_task(
        _heartbeat_loop(
            WorkerKind.AI_ARCHITECT,
            worker_id,
    ...
    try:
        while True:
            job_id = await _next_job_id(worker_id)
            if job_id is None:
                await asyncio.sleep(1)
                continue
            await process_brief_job(job_id)
    finally:
...
async def _next_job_id(_: str) -> UUID | None:
    async with AsyncSessionFactory() as session:
        return (
            await session.execute(
                select(AIJob.id)
                .where(AIJob.status == AIJobStatus.QUEUED)
                .order_by(AIJob.priority, AIJob.available_at, AIJob.created_at)
                .limit(1)
            )
        ).scalar_one_or_none()
